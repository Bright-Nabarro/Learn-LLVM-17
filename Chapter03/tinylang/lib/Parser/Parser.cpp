...
bool Parser::parseImport()
{
	auto _errorhandler = [this]
	{
		return skipUntil(tok::kw_BEGIN, tok::kw_CONST, tok::kw_END,
						 tok::kw_FROM, tok::kw_IMPORT, tok::kw_PROCEDURE,
						 tok::kw_VAR);
	};
	IdentList Ids;
...
bool Parser::parseProcedureDeclaration(DeclList& ParentDecls)
{
	...
	if (expect(tok::identifier))
		return _errorhandler();
	ProcedureDeclaration* D = Actions.actOnProcedureDeclaration(
...
bool Parser::parseIdentList(IdentList& Ids)
{
	...
	while (Tok.is(tok::comma))
	{
		...
		if (expect(tok::identifier))
			return _errorhandler();
		Ids.push_back(std::pair<SMLoc, StringRef>(Tok.getLocation(),
												  Tok.getIdentifier()));
		advance();
	}
	return false;
